...
describe('<NotificationsStorage>', function () {
    ...
    beforeEach(async () => {
        storage = new NotificationsStorage(pool.connection());

        const cp = await pool.connection();
        const r = cp.request();

        try {
            await r.query('TRUNCATE TABLE notificationTasks');
        } catch (e) { /* surpress */ }
        try {
            await r.query('TRUNCATE TABLE notificationCampaigns');
        } catch (e) { /* surpress */ }
        try {
            await r.query('TRUNCATE TABLE notificationSubscribtions');
        } catch (e) { /* surpress */ }
    ...
    describe('#pushTasks()', () => {
        ...
        it.only('should insert a task and assign an id to it', async () => {
